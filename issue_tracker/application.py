...
def fetch_issue_comments(app, number):
    max_comments = app.config["GLM_MAX_COMMENTS"]
    if max_comments <= 0:
        return []

    url = (
        f"https://api.github.com/repos/{app.config['GITHUB_REPOSITORY']}"
    ...
    while url and len(comments) < max_comments:
        ...
        try:
            ...
            if not isinstance(payload, list):
                raise RuntimeError("GitHub 评论接口返回了无效数据")
            comments.extend(payload[: max_comments - len(comments)])
            url = next_link_url(response.headers.get("Link"))
            params = None
        finally:
            response.close()
    return comments
...
def create_app(test_config=None):
    ...
    @app.post("/api/issues/<int:number>/ai-analysis")
